...
export const InstructionsPanel: React.FC<InstructionsPanelProps> = ({
  ...
  const mdClass =
    'prose prose-invert prose-sm max-w-none prose-p:text-slate-400 prose-strong:text-slate-200 prose-code:text-sky-300 prose-code:bg-slate-800 prose-code:px-1 prose-code:rounded prose-headings:text-slate-200 prose-li:text-slate-400 prose-ul:my-1 prose-ol:my-1 prose-p:my-1'
  const hintMdClass =
    'prose prose-invert prose-sm max-w-none prose-p:text-slate-400 prose-p:my-0 prose-code:text-sky-300 prose-code:bg-slate-800 prose-code:px-1 prose-code:rounded [&>p]:inline'
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="space-y-4 pt-4 border-t border-slate-700">
        ...
        >
          ...
          <ChevronDown
            className={`w-4 h-4 transition-transform ${phase2Open ? '' : '-rotate-90'}`}
          />
        </button>
          ...
          (phaseUnlocked ? (
            ...
            <div className="p-4 bg-slate-900 rounded-lg border border-dashed border-slate-700">
              <p className="text-sm text-slate-400">
                Complete Phase 1 to unlock the investigation phase. You'll
                analyze traces to find the root cause!
              </p>
            </div>
          ))}
      </div>
