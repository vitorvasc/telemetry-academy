...
const STATUS = {
  ...
  error: {
    ...
    text: 'text-red-400',
    badge: 'ERR',
    badgeCls: 'bg-red-900/60 text-red-300',
  },
}
          ...
          return (
            <div key={span.id}>
              ...
              >
                ...
                {/* Duration */}
                <div
                  className={`w-16 flex-shrink-0 text-right pr-3 text-xs font-mono font-medium ${s.text}`}
                >
              ...
              {isOpen && (
                <div className="mx-4 mb-2 rounded-lg border border-slate-700 bg-slate-900 overflow-hidden">
                  <div className="px-3 py-1.5 bg-slate-800/60 border-b border-slate-700 flex items-center justify-between">
                    ...
                    <span
                      className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${s.badgeCls}`}
                    >
                      {s.badge}
                    </span>
                  </div>
                  <div className="p-3 space-y-1.5 max-h-48 overflow-y-auto">
                    {Object.entries(span.attributes).map(([k, v]) => (
                      <div key={k} className="flex gap-3 font-mono text-xs">
                        <span className="text-sky-400 flex-shrink-0 w-48 truncate">
                          {k}
                        </span>
                        <span
                          className={`${
                            k.includes('wait') || k.includes('pool')
