...
export const CaseSolvedScreen: React.FC<CaseSolvedScreenProps> = ({
  ...
  return (
    <div
      className={`
      h-full overflow-y-auto bg-slate-900 p-4 md:p-8
      transition-all duration-500 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}
    `}
    >
      {/* Mobile: DOM order stacks confirmation → review → next case.
          Desktop: review pinned to the right column spanning both rows. */}
      <div className="max-w-lg md:max-w-4xl w-full mx-auto min-h-full flex flex-col gap-8 md:grid md:grid-cols-2 md:items-start md:content-center">
        {/* Confirmation + stats */}
