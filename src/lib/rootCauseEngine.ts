...
const helloSpanRules: RootCauseRule[] = [
  // Option A: Missing index (distractor)
  {
    ...
    specificHint:
      '💡 Hint: Look at db.connection_pool.wait_ms — is the time spent waiting for a connection or executing the query?',
  ...
  // Option C: Cache is slow (distractor)
  {
    ...
    specificHint:
      '💡 Hint: Compare the duration of cache.invalidate to db.query in the trace. Which span is actually slow?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
  ...
  // Option D: CPU-bound (distractor)
  {
    ...
    specificHint:
      '💡 Hint: Look for spans showing wait time vs computation time. What does db.connection_pool.wait_ms tell you about where time is being spent?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const dbSpan = findDbQuerySpan(data)
...
const autoMagicRules: RootCauseRule[] = [
  ...
  },
  {
    ...
    specificHint:
      '💡 Hint: Look at which spans are in the trace. Are there any database spans, or is the error on an outbound HTTP call?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const errorSpan = data.spans.find(span => span.status === 'error')
  ...
  },
  {
    ...
    specificHint:
      "💡 Hint: Look at the error span's http.status_code attribute. Is this a timeout or a server-side response?",
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const totalMs = data.totalDurationMs
  ...
  },
  {
    id: 'd',
    label:
      'Network routing issue — DNS resolution failing for the payment service',
    specificHint:
      '💡 Hint: A DNS failure would mean no HTTP response at all. Check the http.status_code attribute on the error span.',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const errorSpan = data.spans.find(span => span.status === 'error')
...
const collectorRules: RootCauseRule[] = [
  {
    id: 'a',
    label:
      'tail_sampling sampling_percentage too low — dropping 99% of traces including all errors',
    evaluate: () => true, // Always correct for this case (YAML-based, no live spans)
    explainCorrect: () =>
      '✓ Correct! The tail_sampling processor had sampling_percentage=1, dropping 99% of traces. Error spans — which should always be kept — were sampled out. Fix: use a policy that explicitly keeps ERROR status spans.',
    explainIncorrect: () => '',
  },
  {
    id: 'b',
    label:
      'OTLP exporter endpoint misconfigured — traces sent to wrong address',
    specificHint:
      '💡 Hint: Consider at what stage in the pipeline sampling vs. exporting happens. Would a wrong endpoint explain why only error traces are missing?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: () =>
      'Not quite. A wrong endpoint causes export failures, but traces would still be sampled. The selective dropout of error spans points to tail_sampling, which runs before export.',
  },
  {
    id: 'c',
    label: 'Receivers not configured — no telemetry reaching the collector',
    specificHint:
      '💡 Hint: Some traces are getting through — just not the error ones. Would a receiver misconfiguration explain that selective pattern?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: () =>
      'Not quite. Missing receivers would drop ALL traces, not just error traces. The selective gap — only errors missing — is the signature of tail_sampling filtering.',
  },
  {
    id: 'd',
    label:
      'Batch processor buffer overflow — high-throughput traces queued and dropped',
    specificHint:
      '💡 Hint: Buffer overflow drops traces randomly under load. What kind of processor would selectively drop only error-status traces?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: () =>
      'No. Buffer overflow drops traces uniformly under load. Here, only error spans are missing — that selective behavior is tail_sampling with a misconfigured error policy, not random buffer overflow.',
  },
]
...
const brokenContextRules: RootCauseRule[] = [
  {
    ...
    specificHint:
      '💡 Hint: Look at the payment.charge span duration — is it actually slow?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
  ...
  },
  {
    ...
    specificHint:
      '💡 Hint: Does the trace show a db.query span, and what is its status?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
...
const theBaggageRules: RootCauseRule[] = [
  {
    ...
    specificHint:
      '💡 Hint: Look at the rate_limiter.check span — did plan-based logic run? What does baggage.user_plan tell you?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
  ...
  },
  {
    ...
    specificHint:
      '💡 Hint: Does the trace show all users being affected, or only this premium user? Check the rate_limit.applied attribute.',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const rlSpan = data.spans.find(s => s.name === 'rate_limiter.check')
  ...
  },
  {
    ...
    specificHint:
      '💡 Hint: Look at the auth.validate span — what did the database return? Did the database lookup even fail?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const authSpan = data.spans.find(s => s.name === 'auth.validate')
...
const metricsTracesRules: RootCauseRule[] = [
  {
    ...
    specificHint:
      '💡 Hint: Look at the checkout.handle span duration and its children. Are there any db.query child spans?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
  ...
  },
  {
    ...
    specificHint:
      '💡 Hint: Look at the api.request span offsetMs — how much time passes before checkout.handle starts?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
  ...
  },
  {
    ...
    specificHint:
      '💡 Hint: Does the trace contain a payment span? What is the checkout.handle span status?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const handlerSpan = data.spans.find(s => s.name === 'checkout.handle')
  ...
  },
  {
    ...
    explainCorrect: (data: Phase2Data) => {
      ...
      const bytes =
        serSpan?.attributes?.['serialization.payload_bytes'] ?? '284621'
      const library =
        serSpan?.attributes?.['serialization.library'] ??
        'google.protobuf (unoptimized)'
...
const logDetectiveRules: RootCauseRule[] = [
  {
    ...
    specificHint:
      '💡 Hint: Look at the db.query span — what is its status and how many rows were affected?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
  ...
  },
  {
    ...
    explainCorrect: (data: Phase2Data) => {
      ...
      const result =
        billingSpan?.attributes?.['billing.result'] ?? 'insufficient_funds'
  ...
  },
  {
    ...
    specificHint:
      '💡 Hint: Look at the payment.gateway span — what is its status and gateway.response_code?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      ...
      const reason =
        gwSpan?.attributes?.['gateway.decline_reason'] ?? 'insufficient_funds'
  ...
  },
  {
    ...
    specificHint:
      '💡 Hint: Check the billing.currency attribute on the billing.charge span. Is the currency the problem?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const billingSpan = data.spans.find(s => s.name === 'billing.charge')
...
const samplingSleuthRules: RootCauseRule[] = [
  // Option A: Aggressive sampling dropped error traces (correct answer)
  {
    ...
    explainCorrect: (data: Phase2Data) => {
      const apiSpan = data.spans.find(s => s.name === 'api.request')
      const checkoutSpan = data.spans.find(s => s.name === 'checkout.handle')
      const collectorSpan = data.spans.find(
        s => s.name === 'collector.pipeline'
      )
      const samplingRate = apiSpan?.attributes?.['sampling.rate'] ?? '0.01'
      const sampledCount =
        checkoutSpan?.attributes?.['sampled_error_traces_count'] ?? '42'
      const estimatedTotal =
        checkoutSpan?.attributes?.['estimated_total_error_traces'] ?? '4200'
      const droppedCount =
        collectorSpan?.attributes?.['collector.dropped_traces_last_hour'] ??
        '415800'
  ...
  // Option B: False positive / alert threshold (distractor)
  {
    ...
    specificHint:
      '💡 Hint: Check collector.dropped_traces_last_hour on the collector.pipeline span. How many traces were dropped?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const collectorSpan = data.spans.find(
      ...
      const dropped =
        ...
        '415800'
      const sampled =
        collectorSpan?.attributes?.['collector.sampled_traces_last_hour'] ??
        '4200'
  ...
  // Option C: User-specific bug (distractor)
  {
    ...
    specificHint:
      '💡 Hint: Check the sampling.rate attribute on the api.request span. Is the low error count a population issue or a sampling issue?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const apiSpan = data.spans.find(s => s.name === 'api.request')
      ...
      const samplingRate = apiSpan?.attributes?.['sampling.rate'] ?? '0.01'
      const errorType =
        checkoutSpan?.attributes?.['error.type'] ?? 'NullPointerException'
      const errorSource =
        checkoutSpan?.attributes?.['error.source'] ?? 'inventory.check'
  ...
  // Option D: Load balancer dropping requests (distractor)
  {
    ...
    specificHint:
      '💡 Hint: Is the api.request span present in the trace? If errors reach the application layer, what else must be causing them to be invisible?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const apiSpan = data.spans.find(s => s.name === 'api.request')
...
const perfectStormRules: RootCauseRule[] = [
  // Option A: Payment bank timeout (distractor)
  {
    ...
    specificHint:
      '💡 Hint: Check the payment.charge span duration and payment.rejection_reason — is it a timeout or a rejection?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const paymentSpan = data.spans.find(s => s.name === 'payment.charge')
      const reason =
        paymentSpan?.attributes?.['payment.rejection_reason'] ?? 'out_of_stock'
  ...
  // Option B: Stale inventory cache (correct answer)
  {
    ...
    explainCorrect: (data: Phase2Data) => {
      ...
      const authSpan = data.spans.find(s => s.name === 'auth.validate')
      const cacheHit =
        inventorySpan?.attributes?.['inventory.cache_hit'] ?? 'false'
      const cachedStock =
        inventorySpan?.attributes?.['inventory.cached_stock'] ?? '0'
      const actualStock =
        inventorySpan?.attributes?.['inventory.actual_stock'] ?? '142'
      const cacheAgeMs =
        inventorySpan?.attributes?.['inventory.cache_age_ms'] ?? '86400000'
      const rejectionReason =
        paymentSpan?.attributes?.['payment.rejection_reason'] ?? 'out_of_stock'
      const poolWait =
        authSpan?.attributes?.['auth.connection_pool.wait_ms'] ?? '3200'
      const poolWaiting = authSpan?.attributes?.['auth.pool_waiting'] ?? '47'
      const cacheAgeDays = Math.round(
        parseInt(cacheAgeMs, 10) / (1000 * 60 * 60 * 24)
      )
  ...
  // Option C: Auth memory leak (distractor)
  {
    ...
    specificHint:
      '💡 Hint: Check auth.result — did auth fail? Look at the inventory.check span for the actual trigger.',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const authSpan = data.spans.find(s => s.name === 'auth.validate')
  ...
  // Option D: Payment double-validation (distractor)
  {
    ...
    specificHint:
      '💡 Hint: Count the inventory.check spans in the trace. How many times was inventory queried?',
    evaluate: () => false,
    explainCorrect: () => '',
    explainIncorrect: (data: Phase2Data) => {
      const inventorySpans = data.spans.filter(
        s => s.name === 'inventory.check'
      )
